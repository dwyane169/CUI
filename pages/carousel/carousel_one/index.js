Page({
  ...
  // 初始化towerSwiper
  towerSwiper(name) {
    let towerList = this.data[name];

    for(let i in towerList){
      towerList[i].zIndex = parseInt(towerList.length / 2) + 1 - Math.abs(i - parseInt(towerList.length / 2))
      towerList[i].mLeft = i - parseInt(towerList.length / 2)
    }
    this.setData({ towerList })
  },
  // towerSwiper触摸开始
  towerStart(e) {
    this.setData({
